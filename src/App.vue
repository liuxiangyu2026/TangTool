<template>
  <div class="flex h-screen overflow-hidden bg-surface text-neutral-900">
    <aside class="flex shrink-0 flex-col border-r border-neutral-200 bg-neutral-50 p-2" :class="preferences.sidebarCollapsed ? 'w-16' : 'w-56'" aria-label="侧栏">
      <img v-if="preferences.sidebarCollapsed" src="/tangtool.svg" alt="TangTool 模块 T Logo" class="mx-auto mb-1 mt-1 h-8 w-8 shrink-0" />
      <div class="mb-4 flex h-11 shrink-0 items-center" :class="preferences.sidebarCollapsed ? 'justify-center' : 'justify-between px-2'">
        <div v-if="!preferences.sidebarCollapsed" class="flex items-center gap-2">
          <img src="/tangtool.svg" alt="TangTool 模块 T Logo" class="h-8 w-8 shrink-0" />
        ...
        <button type="button" class="rounded-md p-2 text-neutral-500 hover:bg-neutral-200 hover:text-neutral-900"
          :aria-label="preferences.sidebarCollapsed ? '展开侧栏' : '折叠侧栏'" :title="preferences.sidebarCollapsed ? '展开侧栏' : '折叠侧栏'"
      ...
      <RouterLink to="/" class="mb-3 flex shrink-0 items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-neutral-100"
        :class="{ 'justify-center': preferences.sidebarCollapsed }" exact-active-class="bg-neutral-200 text-neutral-900" aria-label="首页" title="首页">
        <House :size="18" class="shrink-0" /><span v-if="!preferences.sidebarCollapsed">首页</span>
      </RouterLink>
      <nav id="tool-navigation" class="min-h-0 flex-1 space-y-4 overflow-y-auto" aria-label="工具导航">
        <section v-for="group in menuGroups" :key="group.id">
          ...
          <div v-show="preferences.sidebarCollapsed || !preferences.collapsedGroups.includes(group.id)" :id="`menu-${group.id}`" class="mt-1 space-y-1">
            <RouterLink v-for="item in group.items" :key="item.path" :to="item.path" class="flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-neutral-100"
              :class="{ 'justify-center': preferences.sidebarCollapsed }" :title="item.label" :aria-label="item.label"
              exact-active-class="bg-neutral-200 text-neutral-900">
              <component :is="item.icon" :size="18" class="shrink-0" />
              <span v-if="!preferences.sidebarCollapsed" class="min-w-0 truncate">{{ item.label }}</span>
            </RouterLink>
          </div>
        </section>
      </nav>
      <div class="mt-3 shrink-0 space-y-1 border-t border-neutral-200 pt-2">
        <button v-if="!preferences.sidebarCollapsed" type="button"
          ...
          <ListTree :size="14" />
          {{ allGroupsCollapsed ? "展开全部分组" : "折叠到一级" }}
        </button>
        <RouterLink v-for="item in footerItems" :key="item.path" :to="item.path"
          ...
          exact-active-class="bg-neutral-200 text-neutral-900" :title="item.label" :aria-label="item.label">
...
const footerItems = [
  { path: "/releases", label: `版本与升级 · ${version}`, icon: History },
