...
export function compareText(left: string, right: string, ignoreWhitespace: boolean): TextDiffResult {
  if (left.length > 200000 || right.length > 200000) throw new Error(t('每侧最多支持 20 万字符。'));
  // 保留末尾空行来显示文件结尾换行差异；CRLF / LF 统一，不把系统换行差异当作改动。
  const originalLeft = left === "" ? [] : left.replace(/\r\n?/g, "\n").split("\n");
  const originalRight = right === "" ? [] : right.replace(/\r\n?/g, "\n").split("\n");
  if (originalLeft.length > 2000 || originalRight.length > 2000) throw new Error(t('每侧最多支持 2000 行，请分段对比。'));
  ...
  if (!changes) throw new Error(t('差异计算超出限制，请缩小文本范围。'));
  let l = 0;
  let r = 0;
  let added = 0;
  let removed = 0;
