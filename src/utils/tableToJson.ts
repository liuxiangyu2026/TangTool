...
export type TableJsonResult = { names: string[]; selected: string; json: string; rows: number; notes: string[] };
...
export function tableToJson(request: TableJsonRequest): TableJsonResult {
  ...
  if (csv) {
    try {
      ...
      throw new Error("CSV 不是有效 UTF-8，请先另存为 UTF-8 CSV。");
    }
  }
  const workbook = read(source, { type: csv ? "string" : "array", raw: csv, cellDates: false, cellText: true, sheetRows: 10002 });
  if (!workbook.SheetNames.length || workbook.SheetNames.length > 50) throw new Error("工作表为空或超过 50 个，请拆分文件。");
  ...
  for (const name of request.allSheets ? names : [selected]) {
    ...
    if (range.e.r - range.s.r + 1 > 10001 || range.e.c - range.s.c + 1 > 200) {
      throw new Error(`工作表 ${name} 超出限制（最多 10000 行数据、200 列），未导出截断结果。`);
    }
    ...
    const headers = Array.from({ length: width }, (_, index) => {
      ...
      used.add(key);
      if (request.header && key !== String(cell ?? "")) notes.push(`${name}：第 ${index + 1} 列使用字段名 ${key}`);
      return key;
    });
    totalRows += rows.length;
    if (totalRows > 20000) throw new Error("本次总数据超过 20000 行，请按工作表分别导出。");
  ...
  if (json.length > 5_000_000) throw new Error("JSON 结果超过 500 万字符，请缩小表格范围。");
