export function parseTimestamp(input: string, unit: "seconds" | "milliseconds"): number {
  if (!/^-?\d+$/.test(input.trim())) throw new Error("请输入整数时间戳，并选择秒或毫秒。");
...
export function parseIsoDate(input: string): number {
  ...
  if (!parts) throw new Error("使用带时区的 ISO 时间，例如 2026-09-14T12:00:00+08:00 或 2026-09-14T04:00:00Z。");
  const [year, month, day, hour, minute, second] = parts.slice(1, 7).map(Number);
  const leap = year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0);
  const days = [31, leap ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  const zone = parts[8];
  if (month < 1 || month > 12 || day < 1 || day > days[month - 1] || hour > 23 || minute > 59 || second > 59) {
    throw new Error("日期或时间不存在，请检查月份、天数和时分秒。");
  }
  if (zone !== "Z" && (Number(zone.slice(1, 3)) > 23 || Number(zone.slice(4)) > 59)) {
    throw new Error("时区偏移无效。");
  }
  const value = Date.parse(input.trim());
  if (!Number.isFinite(value)) throw new Error("无法解析日期时间。");
  return value;
}

export function generateUuids(count: number): string[] {
  if (!Number.isInteger(count) || count < 1 || count > 1000) throw new Error("生成数量需要是 1～1000 的整数。");
  if (!globalThis.crypto?.getRandomValues) throw new Error("当前环境不支持安全随机数，无法生成 UUID。");
