<template>
  <section class="json-result-editor flex min-h-48 flex-1 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-surface" aria-label="JSON 结果区域">
    <header class="flex shrink-0 items-center justify-between gap-3 border-b border-neutral-200 px-3 py-2 text-xs">
      <h2 class="font-medium text-neutral-700">JSON 结果</h2>
      <div class="flex items-center gap-3">
        <button type="button" class="flex items-center gap-1 text-blue-700 disabled:opacity-50" :disabled="!value" @click="foldResult(true)"><FoldVertical :size="14" />全部折叠</button>
        <button type="button" class="flex items-center gap-1 text-blue-700 disabled:opacity-50" :disabled="!value" @click="foldResult(false)"><UnfoldVertical :size="14" />全部展开</button>
      </div>
    </header>
...
onMounted(() => {
  ...
  editor = new EditorView({
    ...
    doc: props.value,
    extensions: [
      basicSetup, editorPreferences(), json(), syntaxHighlighting(jsonHighlightStyle), jsonEditorTheme,
      EditorState.readOnly.of(true), EditorView.editable.of(false),
      EditorView.contentAttributes.of({ "aria-label": "表格转 JSON 结果", "aria-readonly": "true", role: "textbox", tabindex: "0" }),
