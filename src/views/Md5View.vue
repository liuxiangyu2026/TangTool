      ...
      <SplitPane direction="horizontal" :label="t('调整MD5 输入和摘要宽度')">
        ...
        <template #second>
          <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-surface">
            ...
            <textarea class="min-h-0 flex-1 resize-none bg-neutral-50 p-3 font-mono text-sm" :aria-label="t('MD5 结果')" :value="busy ? t('正在计算，请稍候…{p0}', { p0: progress === null ? '' : ` ${progress}%` }) : result" readonly :placeholder="t('摘要会显示在这里')"></textarea>
