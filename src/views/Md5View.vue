  ...
  <section class="flex h-full min-h-0 flex-col bg-neutral-100 p-4 sm:p-6">
    ...
    <div class="mt-4 flex min-h-0 flex-1 flex-col gap-4">
      <div class="flex shrink-0 flex-wrap gap-2 rounded-lg border border-neutral-200 bg-surface px-3 py-2">
        <button class="rounded-md bg-violet-600 px-3 py-1.5 text-sm text-white" type="button" @click="kind === 'text' ? (input = 'TangTool 文本摘要示例') : selectFile()">
          示例/选择文件
        </button>
        <button class="rounded-md bg-blue-600 px-3 py-1.5 text-sm text-white disabled:opacity-50" type="button" :disabled="kind === 'file' ? filePath === '' : false"
          @click="kind === 'text' ? calculateText() : calculateFile()">
          计算 MD5
        </button>
      ...
      <SplitPane direction="horizontal" label="调整MD5 输入和摘要宽度">
        ...
        <template #second>
          <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-surface">
            ...
            <textarea class="min-h-0 flex-1 resize-none bg-neutral-50 p-3 font-mono text-sm" aria-label="MD5 结果" :value="result" readonly placeholder="摘要会显示在这里"></textarea>
...
async function calculateText() {
  try {
    result.value = await invoke<string>("calculate_text_md5", { input: input.value });
    error.value = "";
    status.value = "计算完成";
  } catch (e) {
    result.value = "";
    error.value = typeof e === "string" ? e : "计算失败";
  }
}

async function calculateFile() {
  try {
    const value = await invoke<{ digest: string }>("calculate_file_md5", { path: filePath.value });
    result.value = value.digest;
    error.value = "";
    status.value = "计算完成";
  } catch (e) {
    result.value = "";
