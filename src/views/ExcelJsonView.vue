<template>
  <ToolPage description="导入 XLSX / XLS / UTF-8 CSV，选择工作表转 JSON。空单元格为 null；合并单元格只保留左上角值，不执行公式或宏。" :error="error" :status="status">
    <template #actions>
      <label class="tool-button cursor-pointer bg-violet-600">
        <FolderOpen :size="14" />
        选择表格
        <input type="file" class="sr-only" accept=".xlsx,.xls,.csv" aria-label="选择表格文件" :disabled="busy" @change="selectFile" />
      </label>
    ...
    <SplitPane direction="vertical" label="调整表格选项和 JSON 结果高度" :initial="32" :min="20" :max="60">
      <template #first>
        <div class="tool-panel space-y-3 text-sm">
          <p class="break-all">{{ filename || "尚未选择文件（最大 10 MiB）" }}</p>
          ...
          <p class="text-xs leading-5 text-neutral-500">
            默认保留 Excel 数值/布尔类型，日期为 Excel 序列号；勾选“使用显示文本”可保留日期、百分比等显示格式。CSV
            默认按文本读取，保留前导零。空白行跳过，重名字段自动加后缀。
          </p>
          <p class="h-5 truncate text-xs text-neutral-500" :title="notes.join('；')">{{ notes.join("；") || "字段名调整说明将在这里显示。" }}</p>
        </div>
      </template>
      <template #second>
...
async function selectFile(event: Event) {
  ...
  if (!/\.(xlsx|xls|csv)$/i.test(file.name) || file.size > 10 * 1024 * 1024) {
    error.value = "请选择 10 MiB 以内的 XLSX、XLS 或 UTF-8 CSV 文件。";
    return;
  }
  const id = revision;
  busy.value = true;
  try {
    ...
    if (id === revision) error.value = "读取文件失败。";
  } finally {
...
async function convert() {
  ...
  try {
    ...
    status.value = `已转换 ${value.rows} 行数据`;
    busy.value = false;
  } catch (reason) {
    if (id === revision) error.value = reason instanceof Error ? reason.message : "转换失败。";
  } finally {
    if (id === revision) busy.value = false;
  }
}
...
async function exportJson() {
  ...
  try {
    ...
    status.value = saved ? "JSON 已保存" : "已取消保存";
  } catch {
