<template>
  <ToolPage description="按 UTF-8 字节计算文本 SHA-256 / SHA-512。空文本也有合法摘要；摘要不是加密，不能直接用于保存密码。本页暂不处理文件。" :error="error" :status="status">
    <template #actions>
    ...
    <label class="tool-panel flex items-center gap-3 text-sm">
      ...
      <span class="text-xs text-neutral-500">不自动裁剪空格或换行，最多 2 MiB</span>
    </label>
    <SplitPane direction="vertical" label="调整SHA 输入和摘要高度" :initial="65" :min="30" :max="80">
      <template #first>
        <textarea v-model="input" class="tool-editor" aria-label="SHA 文本输入" placeholder="输入需要计算摘要的文本"></textarea>
      </template>
      <template #second>
        <div class="tool-panel flex min-h-0 flex-col">
          <h2 class="mb-2 text-sm font-medium">十六进制摘要</h2>
          <textarea class="min-h-0 flex-1 w-full resize-none font-mono text-sm" :value="result" readonly rows="3" aria-label="SHA 摘要结果" placeholder="结果显示在这里"></textarea>
        </div>
      </template>
    </SplitPane>
  </ToolPage>
...
async function calculate() {
  ...
  if (input.value.length > 2 * 1024 * 1024) {
    error.value = "文本过大，最多处理 2 MiB UTF-8 内容。";
    return;
  }
  ...
  try {
    ...
    if (id === requestId) error.value = "无法计算摘要，请确认当前环境支持 Web Crypto。";
  } finally {
