<template>
  <ToolPage description="显式区分秒 / 毫秒；日期输入必须带时区偏移。只在本机转换，不访问网络时间服务。" :error="error" :status="status">
    <template #actions>
    ...
    <div class="tool-panel space-y-3">
      <label for="timestamp-input" class="block text-sm font-medium">时间戳 → 日期</label>
      <div class="flex flex-wrap gap-2">
        <input id="timestamp-input" v-model="timestamp" class="tool-field min-w-0 flex-1 font-mono" placeholder="例如 0" @input="invalidate" />
        <select v-model="unit" class="tool-field" aria-label="时间戳单位" @change="invalidate">
    ...
    <div class="tool-panel space-y-3">
      <label for="iso-input" class="block text-sm font-medium">日期 → 时间戳</label>
      <div class="flex flex-wrap gap-2">
    ...
    <div class="tool-panel space-y-3">
      ...
      <p class="text-xs text-neutral-500">时区只影响显示，不改变时间戳；秒数向下取整，毫秒保留精度。</p>
      <pre class="whitespace-pre-wrap break-all font-mono text-sm leading-7" aria-label="时间转换结果">{{ result || "转换结果显示在这里" }}</pre>
    </div>
  </ToolPage>
</template>
...
const result = computed(() => {
  ...
  return `秒：${Math.floor(instant.value / 1000)}\n毫秒：${instant.value}\nUTC：${date.toISOString()}\n${timeZone.value}：${display}`;
});
...
function convert(source: "timestamp" | "date") {
  ...
  try {
    ...
    error.value = reason instanceof Error ? reason.message : "转换失败";
  }
}
