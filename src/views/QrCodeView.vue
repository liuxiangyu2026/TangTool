<template>
  <ToolPage :description="t('本地生成 QR Code，不会打开或上传输入的链接。支持文本与网址；留出四格白边以便识别，保存 PNG 或 SVG。')" :error="error" :status="status">
    <template #actions>
    ...
    <div class="tool-panel flex flex-wrap items-center gap-4 text-sm">
      ...
      <span class="text-xs text-neutral-500">{{ t('默认 M；等级越高，图案通常越密。最多 1000 个 UTF-8 字节。') }}</span>
    </div>
    <SplitPane direction="horizontal" :label="t('调整二维码输入和预览宽度')">
      <template #first>
        <textarea v-model="input" class="tool-editor h-full" :aria-label="t('二维码内容')" :placeholder="t('输入文本或网址')"></textarea>
      </template>
      <template #second>
        <div class="tool-panel flex flex-col items-center justify-center gap-3">
          <img v-if="png" :src="png" :alt="t('生成的二维码预览')" class="max-h-80 max-w-full" style="image-rendering: pixelated" />
          <p v-else class="text-sm text-neutral-500">{{ t('生成后显示二维码') }}</p>
          <p v-if="png" class="text-xs text-neutral-500">{{ t('请实际扫码确认内容，再用于印刷或分享。') }}</p>
        </div>
      </template>
    </SplitPane>
  </ToolPage>
...
async function generate() {
  ...
  if (!input.value.trim() || new TextEncoder().encode(input.value).byteLength > 1000) {
    error.value = t('请输入非空内容，最多支持 1000 个 UTF-8 字节。');
    return;
  }
  const id = revision;
  busy.value = true;
  try {
    ...
    png.value = image;
    svg.value = vector;
    status.value = t('二维码已生成');
  } catch {
    if (id === revision) error.value = t('生成失败，请缩短内容或降低纠错等级。');
  } finally {
...
async function exportCode(format: "png" | "svg") {
  ...
  try {
    ...
    status.value = saved ? t('二维码已保存') : t('已取消保存');
  } catch {
