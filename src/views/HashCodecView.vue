<template>
  <section class="flex h-full min-h-0 flex-col bg-neutral-100 p-4 sm:p-6">
    <header class="shrink-0">
      ...
      <div class="mt-4 inline-flex rounded-md border border-neutral-200 bg-white p-1" role="tablist" :aria-label="t('编码工具模式')">
      ...
      <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('输入内容') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('按 URL 参数组件处理，空格会编码为 `%20`。') }}</p>
          </header>
          <textarea v-model="urlInput" class="min-h-0 flex-1 resize-none bg-white p-3 font-mono text-sm leading-6 text-neutral-800 outline-none placeholder:text-neutral-400" :aria-label="t('URL 编码输入')" :placeholder="t('请输入 URL、参数或文本')" spellcheck="false" @input="resetUrlResult"></textarea>
        </section>

        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('处理结果') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('结果只在本机处理，不会发起网络请求。') }}</p>
          </header>
          <textarea class="min-h-0 flex-1 resize-none bg-neutral-50 p-3 font-mono text-sm leading-6 text-neutral-800 outline-none" :aria-label="t('URL 编码结果')" :value="urlResult" :placeholder="t('处理结果会显示在这里')" readonly spellcheck="false"></textarea>
    ...
    <div v-else-if="mode === 'md5'" class="mt-4 flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('文本输入') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('按 UTF-8 字节计算 MD5 摘要。') }}</p>
          </header>
          <textarea v-model="md5Input" class="min-h-0 flex-1 resize-none bg-white p-3 text-sm leading-6 text-neutral-800 outline-none placeholder:text-neutral-400" :aria-label="t('MD5 文本输入')" :placeholder="t('请输入需要计算摘要的文本')" spellcheck="false" @input="resetMd5Result"></textarea>
        </section>

        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('MD5 摘要') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('MD5 是摘要算法，不是加密，不能用于存储密码。') }}</p>
          </header>
          <textarea class="min-h-0 flex-1 resize-none bg-neutral-50 p-3 font-mono text-sm leading-6 text-neutral-800 outline-none" :aria-label="t('MD5 摘要结果')" :value="md5Result" :placeholder="t('MD5 摘要会显示在这里')" readonly spellcheck="false"></textarea>
    ...
    <div v-else class="mt-4 flex min-h-0 flex-1 flex-col gap-4">
      <div class="flex shrink-0 flex-wrap items-center gap-2 rounded-lg border border-neutral-200 bg-white px-3 py-2">
        ...
        <button class="flex items-center gap-1.5 rounded-md bg-blue-600 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60" type="button" :disabled="filePath === '' || isFileCalculating" @click="calculateFileMd5">
          <Hash :size="14" aria-hidden="true" />
          <span>{{ isFileCalculating ? t('计算中') : t('计算 MD5') }}</span>
        </button>
      ...
      <div class="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('文件输入') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('文件只在本机读取，采用流式处理，不会上传。') }}</p>
          </header>
          <div class="min-h-0 flex-1 overflow-auto p-4">
            <button class="flex w-full items-center gap-3 rounded-lg border border-dashed border-neutral-300 bg-neutral-50 px-4 py-4 text-left transition hover:border-blue-400 hover:bg-blue-50" type="button" @click="selectFile">
              ...
              <span class="min-w-0">
                <span class="block text-sm font-medium text-neutral-800">{{ fileName || t('选择需要计算的文件') }}</span>
                <span class="mt-1 block truncate text-xs text-neutral-500">{{ fileName ? t('点击重新选择文件') : t('支持任意本地文件') }}</span>
              </span>
            </button>
          </div>
        </section>
        ...
        <section class="flex min-h-0 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white">
          <header class="shrink-0 border-b border-neutral-200 px-3 py-2">
            <h2 class="text-sm font-medium text-neutral-700">{{ t('文件 MD5 摘要') }}</h2>
            <p class="mt-0.5 text-xs text-neutral-400">{{ t('MD5 是摘要算法，不是加密，不能用于存储密码。') }}</p>
          </header>
          <div class="min-h-0 flex-1 overflow-auto p-4">
            <div class="rounded-lg border border-neutral-200 bg-neutral-50 p-4">
              <p class="break-all font-mono text-sm leading-6 text-neutral-800">{{ fileMd5Result || t('选择文件并点击计算 MD5') }}</p>
            </div>
            <div v-if="isFileCalculating || fileProgress > 0" class="mt-4">
              <div class="flex items-center justify-between text-xs text-neutral-500">
                <span>{{ isFileCalculating ? t('正在读取文件…') : t('处理完成') }}</span>
            ...
            <p v-if="fileByteLength !== null" class="mt-4 text-sm text-neutral-500">{{ t('文件大小：{p0}', { p0: formatFileSize(fileByteLength) }) }}</p>
...
async function copyUrlResult() {
  ...
  try {
    ...
    urlStatus.value = t("已复制到剪贴板");
    urlError.value = "";
  } catch {
    urlError.value = t("复制失败，请重新尝试");
    urlStatus.value = "";
  }
}
...
async function calculateMd5() {
  try {
    ...
    md5Error.value = "";
    md5Status.value = t("MD5 计算完成");
  } catch (error) {
    md5Result.value = "";
    md5Error.value = typeof error === "string" ? error : t("MD5 计算失败，请重新尝试");
    md5Status.value = "";
  }
}
...
async function copyMd5() {
  ...
  try {
    ...
    md5Status.value = t("已复制到剪贴板");
    md5Error.value = "";
  } catch {
    md5Error.value = t("复制失败，请重新尝试");
    md5Status.value = "";
  }
}
...
async function selectFile() {
  try {
    const selected = await open({ title: t('选择需要计算 MD5 的文件'), multiple: false, directory: false });
    ...
    fileError.value = "";
    fileStatus.value = t("已选择文件");
  } catch {
    fileError.value = t("选择文件失败，请确认文件可访问");
    fileStatus.value = "";
  }
}
...
async function calculateFileMd5() {
  ...
  try {
    ...
    fileStatus.value = t("文件 MD5 计算完成");
  } catch (error) {
    fileError.value = typeof error === "string" ? error : t("文件 MD5 计算失败，请确认文件可访问");
  } finally {
...
async function copyFileMd5() {
  ...
  try {
    ...
    fileStatus.value = t("已复制到剪贴板");
    fileError.value = "";
  } catch {
    fileError.value = t("复制失败，请重新尝试");
    fileStatus.value = "";
  }
}
