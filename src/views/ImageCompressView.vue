<template>
  <ToolPage description="单张 JPEG / PNG / WebP 转 JPEG 或 WebP，可调质量和尺寸。默认另存为新文件，不放大原图；动画仅保留首帧，元数据不保留。" :error="error" :status="status">
    <template #actions>
      <label class="tool-button cursor-pointer bg-violet-600">
        <FolderOpen :size="14" />
        选择图片
        <input type="file" class="sr-only" accept="image/jpeg,image/png,image/webp" aria-label="选择图片文件" @change="selectFile" />
      </label>
    ...
    <div class="tool-panel flex flex-wrap items-center gap-4 text-sm">
      <label class="flex items-center gap-2">
        ...
        <select v-model="format" class="tool-field">
          ...
          <option value="image/webp">WebP（保留透明）</option>
        </select>
      </label>
      <label class="flex items-center gap-2">
        质量
    ...
    <p class="text-xs text-neutral-500">JPEG / WebP 为有损压缩；质量越高不一定体积越小。文件最多 20 MiB、2400 万像素。保存前请检查预览。</p>
    <SplitPane direction="horizontal" label="调整原图和压缩预览宽度">
      <template #first>
        <div class="tool-panel flex min-h-0 flex-col gap-3">
          <h2 class="break-all text-sm font-medium">{{ source?.name || "原图" }}</h2>
          <p v-if="source" class="text-xs text-neutral-500">
            {{ formatBytes(source.size) }}
            <span v-if="result">· {{ result.originalWidth }} × {{ result.originalHeight }}</span>
          </p>
          <img v-if="sourceUrl" :src="sourceUrl" alt="原图预览" class="min-h-0 w-full flex-1 object-contain" />
        </div>
      </template>
      <template #second>
        <div class="tool-panel flex min-h-0 flex-col gap-3">
          <h2 class="text-sm font-medium">压缩结果</h2>
          <p v-if="result" class="text-xs text-neutral-500">{{ formatBytes(result.blob.size) }} · {{ result.width }} × {{ result.height }} · {{ sizeMessage }}</p>
          <img v-if="resultUrl" :src="resultUrl" alt="压缩结果预览" class="min-h-0 w-full flex-1 object-contain" />
        </div>
      </template>
    </SplitPane>
  </ToolPage>
...
const sizeMessage = computed(() => {
  ...
  return reduction > 0 ? `减少 ${(reduction * 100).toFixed(1)}%` : "体积未减小，建议保留原图";
});
...
function selectFile(event: Event) {
  ...
  if (!/^image\/(jpeg|png|webp)$/.test(file.type) || file.size > 20 * 1024 * 1024 || file.size === 0) {
    error.value = "请选择非空且不超过 20 MiB 的 JPEG、PNG 或 WebP 图片。";
    return;
  }
  source.value = file;
...
async function run() {
  ...
  try {
    ...
    status.value = "处理完成，检查预览后另存。";
  } catch (reason) {
    if (id === revision) error.value = reason instanceof Error ? reason.message : "图片无法解码或压缩。";
  } finally {
...
async function exportImage() {
  ...
  try {
    ...
    status.value = saved ? "图片已另存" : "已取消保存";
  } catch {
