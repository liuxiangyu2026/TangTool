<template>
  <ToolPage description="批量生成 UUID v4，使用安全随机源。不包含设备标识和时间信息；不作为密码使用。" :error="error" :status="status">
    <template #actions>
    ...
    <div class="tool-panel flex flex-wrap items-center gap-5 text-sm">
      ...
      <span class="text-xs text-neutral-500">每次 1～1000 条，默认 10 条</span>
    </div>
    <textarea class="tool-editor" :value="result" readonly aria-label="UUID 生成结果" placeholder="点击生成，每行一个 UUID"></textarea>
  </ToolPage>
</template>
...
function generate() {
  ...
  try {
    ...
    status.value = `已生成 ${values.value.length} 条 UUID`;
  } catch (reason) {
