...
pub fn convert_file(path: &Path, output_path: &Path, operation: Operation) -> Result<(), String> {
    let input = File::open(path).map_err(|error| format!("读取源文件失败：{error}"))?;
    if !input
        .metadata()
        .map_err(|error| format!("读取源文件信息失败：{error}"))?
        .is_file()
    {
        return Err("请选择普通文件作为输入".to_string());
    }
    let input_handle = Handle::from_file(
        input
            .try_clone()
            .map_err(|error| format!("检查源文件身份失败：{error}"))?,
    )
    .map_err(|error| format!("检查源文件身份失败：{error}"))?;
    ...
    let parent = output_path
        ...
        .canonicalize()
        .map_err(|error| format!("输出目录不存在或不可访问：{error}"))?;
    if !parent.is_dir() {
        return Err("输出位置不是目录".to_string());
    }
    ...
    let mut temporary = Builder::new()
        ...
        .suffix(".tmp")
        .tempfile_in(&parent)
        .map_err(|error| format!("无法在输出目录创建临时文件：{error}"))?;
    ...
    let preparation = (|| {
        ...
        temporary
            .as_file()
            .sync_all()
            .map_err(|error| format!("无法完整写入临时文件：{error}"))?;
        ...
        if !unchanged {
            return Err("目标文件在处理期间发生变化，已停止保存，请重新选择目标".to_string());
        }
        Ok(())
    })();
    ...
    match committed {
        Ok(_) => Ok(()),
        Err(error) => {
            let message = format!("提交结果失败，原文件未主动删除：{}", error.error);
            Err(cleanup_temporary(error.file, message))
        }
    }
}

fn inspect_destination(input: &Handle, path: &Path) -> Result<Option<Destination>, String> {
    ...
    if metadata.file_type().is_symlink() {
        return Err("输出目标是符号链接，请另选普通文件，避免误改链接指向的内容".to_string());
    }
    if !metadata.is_file() {
        return Err("输出目标不是普通文件".to_string());
    }
    let handle =
        Handle::from_path(path).map_err(|error| format!("检查目标文件身份失败：{error}"))?;
    // 比较打开文件的身份，而非只比较路径字符串，覆盖硬链接、大小写和相对路径别名。
    if input == &handle {
        return Err("输入和输出指向同一个文件，请选择不同的输出文件".to_string());
    }
    let metadata = handle
        .as_file()
        .metadata()
    ...
    if !metadata.is_file() || metadata.permissions().readonly() {
        return Err("目标文件不是可写的普通文件，请另选保存位置".to_string());
    }
    Ok(Some(Destination {
        handle,
        length: metadata.len(),
        modified: metadata
            .modified()
            .map_err(|error| format!("读取目标修改时间失败：{error}"))?,
...
fn convert_stream(
    ...
    match operation {
        Operation::Encode => {
            ...
            encoder
                .finish()
                .map_err(|error| format!("Base64 编码收尾失败：{error}"))?;
        }
        Operation::Decode => {
    ...
    output
        .flush()
        .map_err(|error| format!("写入结果失败：{error}"))
}

fn cleanup_temporary(file: NamedTempFile, reason: String) -> String {
    let path = file.path().to_path_buf();
    match file.close() {
        Ok(()) => reason,
        Err(error) => format!(
            "{reason}；临时文件清理失败，请手动检查 {}：{error}",
            path.display()
        ),
    }
}
