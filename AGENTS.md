...
- 当前任务：M1-1B-3 为侧栏添加当前路由选中状态；开始前先清理 M1-1B-2 的提交前格式细节
...
- 待提交前清理：`src/App.vue` 文件末尾补换行；菜单数据字符串统一为项目现有的双引号风格；这两项不影响运行，但应在提交前完成
...
- 下一步：完成上述两个格式细节后，继续 M1-1B-3，实现当前路由在侧栏中的清晰选中状态
