...
- 当前任务：M3-3 设计 JSON 转 Excel 的嵌套展平规则
...
- 待提交前清理：当前 M3-1、M3-2 的依赖、纯函数、页面、路由和进度文档尚未提交
...
- 交接基线：最新远程提交是 `18e6aec feat: complete JSON formatting tool`；当前 M3-1 修改应与本文档一起形成下一次提交并推送
- 下一步：明确对象、数组、空值和混合类型的 Excel 行列映射规则，再实现不依赖 Vue 的展平纯函数
...
### 当前任务：M3-3 JSON 转 Excel 嵌套展平规则

- 明确顶层对象、对象数组和基本类型数组如何映射为行
- 明确嵌套对象的列名连接规则
- 明确嵌套数组、空数组、null 和混合类型的处理方式
- 规则确认后先实现纯函数，再接入 SheetJS 和文件保存
...
- 当前最新远端提交为 `18e6aec feat: complete JSON formatting tool`；M3-1 的依赖、结构化对比纯函数、页面、路由和本文档更新尚未形成新提交，具体以 `git status --short --branch` 和 `git log -1 --oneline` 为准。
...
下次继续时先阅读本文档第 7、8 节：从 M3-3 JSON 转 Excel 嵌套展平规则开始；M3-1、M3-2 完成后由开发者提交并推送。
