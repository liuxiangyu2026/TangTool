...
双语与官网代码已实现，正在最终验收/重新打包：`src/i18n/core.ts` 使用可读消息标识、纯文本字典和显式参数；Vue 响应式语言由 `src/i18n/index.ts` 接入，Worker 显式接收语言。Rust 传 `AppError { key, parameters, cause }`，前端统一翻译错误。桌面/官网共用 `src/data/toolCatalog.ts`。不翻译用户输入、文件名或文档/JSON 输出，不通过切换路由 key 重建工具。维护规则见 `docs/localization.md`。
...
3. 按用户已确认的未签名／未公证方案，完善双语、官网工具列表和端正截图，标明安装提示及已验证范围；完成新包验收后上传 Release 并启用 Pages（`website/README.md`）。
4. 发布完成后再考虑文档原生拖放、ESLint/Prettier 和新工具；不在交付修复中扩充功能范围。

双语范围和未签名首发方式均已确认，不再重复询问。当前仍需新三平台构建及 Windows/Intel 真机验收；原生双语预览因 Mac 锁屏待继续，浏览器检查不能替代。此前 GitHub 权限核对有维护权限，未公开 Release 或启用 Pages。正在准备可追溯的新包和官网，旧 7c1d928 包不包含本轮双语；不要误用。用户原有主题和收藏已恢复、颜色页输入保留，用户 Python 环境未改动。
