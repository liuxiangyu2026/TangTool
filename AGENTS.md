...
当前必须由用户补充：确认首发接受未签名/未公证版本，还是准备正式证书后公开发布（签名选择已再次发出，尚未答复）；配合 Windows/Intel 真机验收。Mac 已解锁，原生控制仍偶发屏幕捕获/焦点错误，重读窗口后已完成本轮复制和解码；不能把控制工具错误直接认定为应用缺陷。此前已只读核对 GitHub 仓库有维护权限、无 Release、Pages 未启用；不是账号权限阻塞。尚未合并 main、公开 Release 或部署 Pages。临时验证样本及 UI 测试内容已清理，临时主题和收藏已恢复，用户环境保留。
