...
每个平台分别记录机器系统版本、芯片/架构、产物 SHA-256、日期和结果。Windows/macOS Intel 未实测。2026-09-15 已完成下列 ARM 原生子项，随后 Mac 再次锁屏；复合项未完成全部步骤时只记录已验证部分。
...
| JSON 导入/复制/导出；Base64 文件正常往返及拒绝覆盖源文件 | 待验收 | Base64 编码保存逐字节通过；解码/复制/覆盖确认待验收 | 待验收 |
...
| 收藏、主题、字号、窗口/分栏记忆及默认导出目录重启恢复 | 待验收 | 收藏跨进程重启通过；其余待验收，临时主题/收藏待恢复 | 待验收 |
| 关闭后输入内容不持久化；覆盖安装/升级/卸载行为 | 待验收 | 待验收 | 待验收 |
| 缺少 WebView2 且断网时安装运行 | 待验收 | 不适用 | 不适用 |

## 正式发布前的用户选择

- 当前解锁后的第一步：把主题恢复为“跟随系统”，取消 JSON 格式化收藏；这两项是本轮临时验收值，因重新锁屏尚未恢复。不要误当作用户主动设置。
