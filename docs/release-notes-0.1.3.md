...
- 仍为未签名／未公证预览版：Windows x64、macOS 14+ Apple Silicon / Intel。没有自动安装更新；升级前请保存内容并退出旧版。
...
Document Runtime remains 1.0.0; existing installations do not need replacement, and in-app links continue to use the available v0.1.2 component downloads. Text processing is limited to 1 million input / 2 million output characters and does not perform CSV/SQL escaping. This preview remains unsigned/unnotarized and does not install updates automatically. Build and checksum checks do not replace target-machine acceptance.
